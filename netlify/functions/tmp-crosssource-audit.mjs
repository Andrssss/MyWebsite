// Disposable read-only audit: cross-source duplicate rows (whitelist-scoped),
// with enough detail (id, first_seen) to build a keep-earliest delete list.
// Read-only. No writes.
import { Pool } from "pg";
import { dupeKey, CROSS_SOURCE_DUPE_SOURCES } from "../../src/lib/crossSourceDupe.mjs";

const TOKEN = "tmp-crosssource-audit-2f61ac";
  ...
  try {
    ...
    for (const g of groups) {
      ...
      for (const loser of sorted.slice(1)) {
        toDelete.push({ id: loser.id, source: loser.source, keepId: keep.id, keepSource: keep.source, key: g.key, first_seen: loser.first_seen });
      }
    }

    // Never delete a row marked applied/interview.
      ...
      JSON.stringify(
        {
          ...
          rowsInGroups: groups.reduce((s, g) => s + g.rows.length, 0),
          toDeleteCount: safeDeletes.length,
          blockedByAppliedFlag: blockedDeletes,
          toDelete: safeDeletes,
        },
        null,
        2
      ),
