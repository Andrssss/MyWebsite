...
export default withTimeout("cron_jobs_KH-background", async () => {
  ...
  try {
    ...
    // GH issue #24 follow-up (2026-09-22): kh was whitelisted in
    // CROSS_SOURCE_DUPE_SOURCES so every OTHER caller checks against its rows,
    // but kh's own ingest never checked back — same gap already fixed for
    // kuka (see cross-source-dupe-coverage memory).
    const crossDupeIndex = await loadCrossSourceDupeIndex(client, "kh", { onlySources: CROSS_SOURCE_DUPE_SOURCES });
    console.log(`[kh] cross-source dupe index: ${crossDupeIndex.keySet.size} keys / ${crossDupeIndex.urlSet.size} urls`);
    ...
    for (const row of dedup) {
      try {
        ...
        if (!knownUrls.has(url) && isCrossSourceUrlDupe(crossDupeIndex, url)) {
          ...
          console.log(`[kh] SKIP exact-url dupe (already on another source) → ${url}`);
          continue;
        }
        if (!knownUrls.has(url) && isCrossSourceDupe(crossDupeIndex, COMPANY_NAME, title)) {
          skippedCrossSourceDupe++;
          console.log(`[kh] SKIP cross-source dupe "${title}" @ ${COMPANY_NAME} → ${url}`);
          continue;
        }
    ...
    console.log(
      ...
      `skipped_senior=${skippedSenior}, skipped_no_title=${skippedNoTitle}, fetch_failed=${detailFetchFailed}, skipped_incomplete=${skippedIncomplete}, skipped_cross_source_dupe=${skippedCrossSourceDupe}`
    );
