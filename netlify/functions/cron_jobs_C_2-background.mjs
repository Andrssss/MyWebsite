...
const _runJob = withTimeout("cron_jobs_C_2-background", async (request) => {
  ...
  if (!debug) {
    ...
    try {
      await enrichExperience({
        ...
        label: "cvcentrum",
        jobName: "cron_jobs_C_2-background",
      });
    } catch (err) {
      console.error("[cron_jobs_C_2-background] experience enrichment failed:", err.message);
    }
