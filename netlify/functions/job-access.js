...
// against the ADMIN_* / LITTLE_ADMIN* env vars, which the client cannot check
// for itself. Everything else under /allasfigyelo is admin-only now — ordinary
// visitors are redirected to pestidev.hu before a single data fetch fires.
//
//   GET → 200 { access: true,  tier: "admin" | "little" }
...
exports.handler = async (event) => {
  ...
  // Cookie tier first (that is what a browser has); an ADMIN_SECRET bearer
  // counts as the full tier, so curl/script access keeps working.
