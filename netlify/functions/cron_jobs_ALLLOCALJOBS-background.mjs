...
async function upsertJob(client, item) {
  ...
  await client.query(
    ...
    ["alllocaljobs", item.title, item.url, experience, item.company || null, withTitleTechnologies(item.technologies, item.title), computeLevel({ title: item.title, experience, source: "alllocaljobs" })]
  );
}
